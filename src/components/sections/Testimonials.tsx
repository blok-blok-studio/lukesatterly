      ...
      <div className="flex items-center gap-3 mt-auto">
        <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0 bg-accent/10">
          {t.image ? (
            <Image
              src={t.image}
              alt={t.imageAlt ?? `${t.name}, client of Luke Satterly personal trainer Berlin`}
              width={40}
              height={40}
        ...
        <div>
          ...
          {t.age && <span className="text-zinc-500 text-xs ml-1.5">{t.age}</span>}
        </div>
      </div>
    </div>
  );
...
const testimonials: Testimonial[] = [
  ...
  { name: "Mario", age: 48, image: "/testimonial-mario-personal-trainer-berlin-client.webp", text: "Luke hat mein Selbstvertrauen gestärkt, indem er mich herausfordert und mich bei meinem Wunsch, trotz Arthrose fit und beweglich zu bleiben, durch gezielte Übungen unterstützt. Er zeigt mir, dass ich Kraft habe und diese auch steigern kann. Deshalb fühle ich mich gut aufgehoben bei ihm." },
  { name: "Andreea", age: 36, image: "/testimonial-andreea-personal-trainer-berlin-client.webp", text: "Starting my training journey with Luke as my PT a year ago has been the best investment I ever made for my health and fitness. He knows just when to push me harder and when to take a step back. Our sessions are more than just workouts. They're a time where I can laugh, let out a bit of frustration, and achieve things I never thought I could." },
  ...
  { name: "Laura", age: 36, image: "/testimonial-laura-personal-trainer-berlin-client.webp", imageAlt: "Laura, 36, personal training and body sculpting client of Luke Satterly in Berlin", text: "Ich habe mich ganz bewusst für Luke entschieden, weil ich meinen Körper gezielt formen und auf ein neues Level bringen wollte. Bei ihm ging es nie nur um Training, sondern darum, ein ganz neues Körpergefühl zu entwickeln. Durch seine detailorientierte Art und seinen hohen Anspruch an saubere Ausführung habe ich gelernt, meinen Körper wirklich zu kontrollieren und Bewegungen bewusst umzusetzen.\n\nLuke hat mir eine klare Struktur gegeben, die perfekt zu mir passt – im Training genauso wie bei der Ernährung. Dadurch fühle ich mich heute stärker, sicherer und deutlich wohler in meinem Körper. Für mich war das kein klassisches Personal Training, sondern ein echtes körperliches und mentales Upgrade." },
  { name: "Markus", age: 41, image: "/testimonial-markus-personal-trainer-berlin-client.webp", text: "Ich bin sehr zufrieden mit Luke. Durch die drei individuell erstellten Trainingspläne (Geräte, Freihanteln und Körpergewicht) kann ich flexibel, ortsungebunden und abwechslungsreich trainieren.\n\nDie Einheiten am frühen Morgen passen perfekt in meinen Alltag. Besonders schätze ich die Kombination aus Fachwissen, Motivation und den präzisen Korrekturen bei der Ausführung.\n\nIch wollte von Anfang an intensiv betreut werden, um eine gute Grundlage zu schaffen und danach selbstständig weitertrainieren zu können – genau dabei hat er mich optimal unterstützt. Klare Empfehlung!" },
];
...
export function Testimonials() {
  ...
  const visibleTestimonials = testimonials.filter((t) => t.text.length > 0);

  const goToDesktop = (idx: number) => {
    const bounded = Math.max(0, Math.min(idx, visibleTestimonials.length - 1));
      ...
      <div ref={ref} className="relative">
        ...
        <div className="hidden md:block relative overflow-hidden">
          ...
          <motion.div
            ...
            drag="x"
            dragDirectionLock
            dragConstraints={{ left: -(visibleTestimonials.length - 1) * DESKTOP_CARD_STEP, right: 0 }}
            dragElastic={0.1}
          ...
          <div className="flex items-center justify-center gap-6 mt-8">
            ...
            <span className="text-zinc-400 text-xs uppercase tracking-wider tabular-nums">
              {desktopIndex + 1} / {visibleTestimonials.length}
            </span>
            <button
              aria-label="Next testimonial"
              onClick={() => goToDesktop(desktopIndex + 1)}
              disabled={desktopIndex >= visibleTestimonials.length - 1}
              className="w-11 h-11 rounded-full border border-white/15 bg-white/5 backdrop-blur hover:bg-accent hover:border-accent transition-colors duration-200 text-white flex items-center justify-center cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="md:hidden relative">
          ...
          <motion.div
            ...
            drag="x"
            dragDirectionLock
            dragConstraints={{ left: -(visibleTestimonials.length - 1) * MOBILE_CARD_STEP, right: 0 }}
            dragElastic={0.1}
            dragTransition={{ bounceStiffness: 300, bounceDamping: 30 }}
            onDragEnd={() => {
              const x = mobileX.get();
              const idx = Math.max(0, Math.min(Math.round(-x / MOBILE_CARD_STEP), visibleTestimonials.length - 1));
              setMobileIndex(idx);
          ...
          <p className="text-center text-zinc-400 text-xs mt-6 uppercase tracking-wider tabular-nums">
            {mobileIndex + 1} / {visibleTestimonials.length}
          </p>
        </div>
      </div>
    </section>
